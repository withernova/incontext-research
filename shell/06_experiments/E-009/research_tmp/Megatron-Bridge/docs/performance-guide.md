...
## Dataloader Initialization Performance

1. Dataloader initialization with large multi-dataset blends can take several minutes at scale due to filesystem existence checks, inter-rank synchronization barriers, and memory mapping of index files. Megatron-Bridge inherits several acceleration features from Megatron Core that can reduce initialization time from minutes to seconds on multi-node clusters.

2. Prerequisites: cache path and pre-built caches

   > 1. All acceleration features below require that dataset index caches already exist on disk. Set the cache directory so that index files are written to (and later read from) a shared location:
   >
   >    > 1. `GPTDatasetConfig.path_to_cache=<str>`
   >
   > 2. On the first run without acceleration flags, caches are built and stored at this path. Subsequent runs can then enable the acceleration features below. Use `GPTDatasetConfig.num_dataset_builder_threads=<int>` to parallelize the initial cache build.

3. Fast cache loading

   > 1. When dataset caches already exist, this option skips filesystem existence checks on `.idx`/`.bin` files, removes inter-rank synchronization barriers during dataset construction (so all ranks build datasets in parallel), and skips `os.path.isfile` checks on cached indexes. This yields significant speedups at scale.
   >
   >    > 1. `GPTDatasetConfig.fast_cache_load=True`
   >
   > 2. Constraints:
   >
   >    > 1. `path_to_cache` must be set.
   >    > 2. All caches must have been built by a prior run.
   >    > 3. `blend` must be `None` — in Megatron-Bridge, `data_path` cannot be used with `fast_cache_load` because it is converted to `blend` internally. Configure data via `blend_per_split` instead.

4. Deferred memory mapping of index files

   > 1. Defers the memory mapping of `.npy` index files until first access, moving the cost out of the initialization critical path.
   >
   >    > 1. `GPTDatasetConfig.defer_npy_index_mmap=True`
   >
   > 2. Constraints:
   >
   >    > 1. `path_to_cache` must be set.
   >    > 2. All caches must have been built by a prior run.
   >
   > 3. Best combined with `DataloaderConfig.num_workers > 0` so that DataLoader prefetching hides the deferred memory mapping cost.

5. Precomputed sequence counts per dataset

   > 1. Provides precomputed sequence and document counts for each dataset path, eliminating the need to open and parse per-dataset index file headers during initialization. Especially beneficial when dealing with hundreds to thousands of file prefixes.
   >
   >    > 1. `GPTDatasetConfig.per_dataset_sequences_path=<str>`
   >
   > 2. The JSON file is generated by the `tools/build_sequences_per_dataset.py` script in Megatron-LM. Note that `--per-dataset-sequences-path` is the output path where the script stores the generated file — this is the same path you then pass to `GPTDatasetConfig.per_dataset_sequences_path` for training.
   >
   >    > ```bash
   ...
   > 3. Constraints:
   >
   >    > 1. The tokenizer must have a valid `vocab_size` set.

6. Recommended combination for fastest initialization

   > 1. For maximum speedup, combine all three features with a pre-built cache:
   >
   >    > ```python
   >    > GPTDatasetConfig(
   >    >     seq_length=4096,
   ...
   >    >     per_dataset_sequences_path="/path/to/sequences.json",
...
- `GPTDatasetConfig.defer_npy_index_mmap`
- `GPTDatasetConfig.fast_cache_load`
- `GPTDatasetConfig.num_dataset_builder_threads`
- `GPTDatasetConfig.path_to_cache`
- `GPTDatasetConfig.per_dataset_sequences_path`
