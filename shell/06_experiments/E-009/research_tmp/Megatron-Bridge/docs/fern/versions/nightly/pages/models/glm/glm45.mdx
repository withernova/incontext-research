...
config.optimizer.min_lr = 1e-5
config.model.mtp_num_layers = 1
config.model.mtp_loss_scaling_factor = 0.3  # 0.3 for first 15T tokens, 0.1 after
...
config.checkpoint.pretrained_checkpoint = "/models/glm45-air-106b"
config.train.train_iters = 1000
config.train.global_batch_size = 128
config.train.micro_batch_size = 1
config.scheduler.max_lr = 5e-6
```
...
config = glm45_air_106b_peft_config(peft_scheme="lora")  # or "dora"
config.checkpoint.pretrained_checkpoint = "/models/glm45-air-106b"
config.train.train_iters = 1000
config.train.global_batch_size = 128
config.train.micro_batch_size = 1
config.scheduler.max_lr = 1e-4
...
config = glm45_355b_peft_config(peft_scheme="dora")
config.checkpoint.pretrained_checkpoint = "/models/glm45-355b"
config.train.train_iters = 1000
config.train.global_batch_size = 128
config.train.micro_batch_size = 1
config.scheduler.max_lr = 1e-4
...
config.model.mtp_num_layers = 1  # Number of MTP prediction layers
config.model.mtp_loss_scaling_factor = 0.3  # 0.3 early training, 0.1 later
# Set mtp_num_layers to None or 0 to disable MTP
```
...
config.model.recompute_num_layers = 2
```
...
config.model.expert_model_parallel_size = 16  # Adjust based on GPU count
