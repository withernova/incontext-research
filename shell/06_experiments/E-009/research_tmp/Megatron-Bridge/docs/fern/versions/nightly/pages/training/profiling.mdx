...
`bridge.training.config.ProfilingConfig` is a dataclass that encapsulates profiling-related settings for training. It resides inside the overall `bridge.training.config.ConfigContainer`, which represents the complete configuration for a training run.
...
Recipe users can leverage the `bridge.recipes.run_plugins.NsysPlugin` to configure NSys profiling through NeMo Run executors. The plugin provides a convenient interface for setting up profiling without manually configuring the underlying NSys command.
...
Similar to NSys, recipe users can use the `bridge.recipes.run_plugins.PyTorchProfilerPlugin` for convenient configuration:
...
For comparing two runs, attributing peak memory to specific allocation sites, or drilling into a captured out-of-memory snapshot, see the Memory Snapshot Analysis skill, which bundles scripts for replaying and diffing these files.
