...
The `bridge.training.config.CheckpointConfig` controls model checkpointing behavior, including saving and loading checkpoints, checkpoint formats, and various optimization features.

<Note>
This documentation covers **Megatron-format checkpoints** used during training. For converting between 🤗 Hugging Face and Megatron formats, see the [Bridge Guide](../bridge-guide).
...
**Important**: When using Megatron FSDP (`use_megatron_fsdp=True`), you must set `ckpt_format="fsdp_dtensor"`. Other formats are not compatible with FSDP's sharded parameter layout. See [Megatron Fsdp](megatron-fsdp) for complete FSDP configuration details.
...
- [Megatron Fsdp](megatron-fsdp) - Megatron FSDP configuration and `fsdp_dtensor` format requirements
- [Parallelisms](../parallelisms) - Understanding data and model parallelism strategies
- [Config Container Overview](config-container-overview) - Complete configuration reference
