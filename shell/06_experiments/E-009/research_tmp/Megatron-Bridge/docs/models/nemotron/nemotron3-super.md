...
### Import HF → Megatron
To import the HF model to your desired `$MEGATRON_MODEL_PATH`, use the
distributed GPU backend because this model uses expert parallelism.
...
- Adjust `--gpus-per-node` based on your available GPUs
