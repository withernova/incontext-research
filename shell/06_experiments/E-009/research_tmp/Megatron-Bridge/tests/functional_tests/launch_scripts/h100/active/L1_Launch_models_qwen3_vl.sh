...
uv run coverage run --data-file="${REPO_ROOT}/.coverage" --source="${REPO_ROOT}" --parallel-mode -m pytest \
  ...
  tests/functional_tests/test_groups/models/qwen_vl/test_qwen3_vl_conversion.py \
  tests/functional_tests/test_groups/models/qwen_vl/test_qwen3_vl_generation.py
coverage combine -q
