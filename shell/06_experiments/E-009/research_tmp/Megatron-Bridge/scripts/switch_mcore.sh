...
ensure_commit() {
    ...
    if git -C "$SUBMODULE_PATH" cat-file -e "${target_commit}^{commit}" 2>/dev/null; then
        return 0
    fi

    log "Pinned $target_name commit ${target_commit:0:12} is not available locally. Fetching from origin..."
    git -C "$SUBMODULE_PATH" fetch --no-tags origin "$target_commit" \
        || err "Failed to fetch $target_name commit $target_commit from origin"

    git -C "$SUBMODULE_PATH" cat-file -e "${target_commit}^{commit}" 2>/dev/null \
        || err "Fetched from origin, but $target_name commit $target_commit is still unavailable"
}
...
do_switch() {
    ...
    ensure_commit "$target_name" "$target_commit"
