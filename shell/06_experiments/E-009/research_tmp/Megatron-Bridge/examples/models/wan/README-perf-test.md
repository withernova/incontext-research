...
```bash
NVTE_FUSED_ATTN=1 uv run python -m torch.distributed.run --nproc_per_node=8 examples/models/wan/pretrain_wan.py \
...
NVTE_FUSED_ATTN=1 uv run python -m torch.distributed.run --nproc_per_node=1 examples/models/wan/inference_wan.py  \
  --task t2v-1.3B \
  --sizes 480*832 \
