...
try:
    ...
    # is_torch_fx_available was removed in newer versions of transformers
...
def _default_rope_init_fn(config, device=None):
    ...
    Provides standard (non-scaled) RoPE initialisation when 'default' is not
    present in ROPE_INIT_FUNCTIONS (removed in transformers >= 4.x).
    """
...
class BailingMoeV2RotaryEmbedding(nn.Module):
    def __init__(self, config: BailingMoeV2Config, device=None):
        ...
        self.config = config
        # "default" was removed from ROPE_INIT_FUNCTIONS in newer transformers; fall back to a
        # standard (non-scaled) implementation.
...
class BailingMoeV2ForCausalLM(BailingMoeV2PreTrainedModel, GenerationMixin):
    # In transformers >= 5.x, _tied_weights_keys must be a dict mapping weight -> source.
    # In earlier versions it was a list; use the new dict format for forward compat.
