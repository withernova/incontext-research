...
if __name__ != "__main__":
    ...
    def log_single_rank(logger: logging.Logger, *args: Any, rank: int = 0, **kwargs: Any):
        """Log a message when running this module as a script."""
        print(*args[1:], **kwargs)
...
def _allocate_auto(
    ...
    for layer_idx in range(total_layers_count):
        if x < 0.5:
            layer_type_list[layer_idx] = Symbols.ATTENTION
    ...
    if mlp_layers_count > 0:
        ...
        for layer_idx in range(total_layers_count):
            if layer_type_list[layer_idx] == Symbols.MAMBA:
                if x < 0.5:
                    layer_type_list[layer_idx] = Symbols.MLP
    ...
    if parallel_layers_count > 0:
        ...
        if remaining_mamba_count > 0:
            if parallel_layers_count >= remaining_mamba_count:
                for layer_idx in range(total_layers_count):
                    if layer_type_list[layer_idx] == Symbols.MAMBA:
                        layer_type_list[layer_idx] = Symbols.PARALLEL
            else:
                mamba_to_parallel_ratio: float = (
                    remaining_mamba_count - parallel_layers_count
                ) / parallel_layers_count
...
def _allocate_override(total_layers_count: int, override_pattern: str) -> list:
    ...
    for layer_symbol in layer_type_list:
        if layer_symbol not in Symbols.VALID:
            raise ValueError(f"In hybrid override pattern, '{layer_symbol}' is not one of {Symbols.VALID}")
...
def allocate_layers(
    ...
    """Allocate layer types from requested hybrid ratios or an override pattern."""
    ...
    layer_type_list = _allocate_auto(
        total_layers_count, target_attention_ratio, target_mlp_ratio, target_parallel_hybrid_ratio
    )
    ...
    if override_pattern is not None:
        ...
        if (
            target_attention_ratio > 0.0 or target_mlp_ratio > 0.0 or target_parallel_hybrid_ratio > 0.0
        ) and not _layer_counts_match(layer_type_list_override, layer_type_list):
            raise ValueError(
        ...
        if layer_type_list_override == layer_type_list:
            log_single_rank(logger, logging.INFO, "The override pattern matches the overridden pattern")
        else:
    ...
    if (
        ...
        or target_mlp_ratio > 0.0
        or target_parallel_hybrid_ratio > 0.0
        or override_pattern is not None
    ):
        ...
        log_single_rank(
            logger,
            logging.INFO,
            f"{actual_attention_layers_count} attention layers in {total_layers_count} total layers.",
        )
        log_single_rank(
            logger,
            logging.INFO,
        ...
        log_single_rank(
            logger,
            logging.INFO,
            f"{actual_parallel_layers_count} parallel layers in {total_layers_count} total layers.",
        )
        log_single_rank(
            logger,
            logging.INFO,
