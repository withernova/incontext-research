...
class Exaone45MultimodalRotaryEmbedding(nn.Module):
    """Multimodal Rotary Embedding for language model.
    only support for Exaone45
    ...
    def __init__(
        ...
        assert not self.rotary_interleaved, "only support Exaone45"
...
# Slightly modified from Exaone45Model.get_rope_index
def get_rope_index(
    ...
    """Different from the original implementation, Exaone45 use timestamps rather than absolute time position ids."""
...
def apply_rotary_pos_emb_absolute(
    ...
    In EXAONE 4.5, the shape of freqs is (seq_length, bs, 1, 2 * dim) instead of [max_seqlen, 1, 1, 2 * dim]
    """
