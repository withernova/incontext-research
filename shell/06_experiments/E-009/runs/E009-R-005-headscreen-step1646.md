# E009-R-005-headscreen-step1646 · 训练后段 checkpoint 1646 的固定样本 head screening
...
## 1. 研究设计
### 研究问题
父训练轨迹在 step 1646 保存的 LoRA checkpoint，是否在同一冻结样本集上产生可与相邻后段 checkpoint 直接比较的 query/reference Top-3 与 Top-5 attention head 集合？
### 本轮目的
只对 step 1646 做一次不更新参数的 teacher-forced head screening；与另外四个后段 checkpoint 保持数据、seed、probe 和 finder 完全一致，用于后续判断找到的 head 是否随训练后段稳定。
### 假设或比较预期
...
五条计划 Run 中唯一变化是 parent_checkpoint；本 Run 固定 step=1646。manifest、100 个样本身份、seed=20260901、row contract、excluded layers、Top-k、finder reward 与 artifact dtype 全部一致。
...
- command: `bash tools/run/run_e009_branch.sh branch.action=head_screen named_run.parent_checkpoint=/defaultShare/archive/liuwenchu/projects/IPLoc/experiments/E-009/E009-focus-qwen3vl8b-lora-1shot-nf4-ddp4/checkpoints/samples_00105340_step_001646 named_run.run_name=head-stability-step1646`
- commit: ``
- workspace: 02
- tmux: incontext-E-009-E009-R-005-headscreen-step1646
- log: /defaultShare/archive/liuwenchu/projects/IPLoc/experiments/E-009/E009-R-005-headscreen-step1646/logs/train.log
- output: /defaultShare/archive/liuwenchu/projects/IPLoc/experiments/E-009/E009-R-005-headscreen-step1646/outputs
...
```json
{
  ...
  "evidence_basis": "只读文件核验确认 /defaultShare/archive/liuwenchu/projects/IPLoc/experiments/E-009/E009-focus-qwen3vl8b-lora-1shot-nf4-ddp4/checkpoints/samples_00105340_step_001646/adapter 下存在 adapter_config.json 与非空 adapter_model.safetensors；当前没有 Solid Run，因此这里只作为可执行输入定位，不作为效果证据。",
  ...
  "model_config": "Qwen3-VL-8B-Instruct + 父训练轨迹 step 1646 的 NF4 LoRA adapter；4 GPU DDP 仅用于并行采集；正常模型保持 SDPA，probe forward 临时切 eager attention。",
